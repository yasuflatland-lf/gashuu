...
/// How far a reader got in a book: the last-viewed leading page index
...
/// AND the resume logic both need, so the unknown/zero-total guard and the 1-based
/// display offset live in ONE place. Immutable value object.
///
/// Semantics note (load-bearing): `last_viewed` stores the *leading page of the
/// last-viewed spread* — the reader's resume position, NOT "the furthest page the
/// reader saw", so it can decrease when the reader turns back. The field name now
/// matches that fact.
...
impl ReadingProgress {
    ...
    pub fn new(last_viewed: usize, total: Option<usize>) -> Self {
    ...
    /// div-by-zero, never NaN/inf); a stale `last_viewed` past `total` clamps to
    /// `1.0`.
    pub fn fraction(&self) -> f32 {
        match self.total {
            Some(t) if t > 0 => (self.last_viewed as f32 / t as f32).clamp(0.0, 1.0),
            _ => 0.0,
        }
    }
...
mod tests {
    ...
    #[test]
    fn is_unread_true_when_last_viewed_zero() {
        let p = ReadingProgress::new(0, Some(10));
        assert!(p.is_unread());
    }

    #[test]
    fn is_unread_false_when_last_viewed_nonzero() {
