...
- Re-opening a book resumes at its stored last page (via `OpenBookUseCase::run` → `jump_to`). Resume/write-back is observable for books opened through the library carousel (Return / center-cover double-click) and the add flow.
...
- **Save is fallible end-to-end** — `Library::to_json -> Result` (symmetric with `from_json`); `LibraryStore::save` propagates via `?`. No serialize step is silently swallowed, so a save can no longer write a truncated file while the UI reports success.
...
- **Save-failure rollback**: if the library save fails after a `remove_many`, the full `Book` clones (captured before removal) are re-inserted via `Library::restore`, returning the shelf to a byte-identical pre-removal state. The selection is preserved so the user can retry; no cache entries are touched on failure.
