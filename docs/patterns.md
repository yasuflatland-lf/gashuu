...
Five surfaces copy the glass anatomy verbatim, with NO shared Slint component for the layers — Slint 1.x exposes no backdrop-blur and the recipe does not reduce to a reusable atom without restructuring all five call sites:

1. **NavBar** (`components/NavBar.slint` ~line 199) — `background: Theme.glass-fill`; solid-fill variant (pill chrome).
2. **SelectionToolbar** (`components/SelectionToolbar.slint` ~line 70) — `background: Theme.glass-fill`; solid-fill variant (pill chrome, verbatim from NavBar lines 189–195 per its own comment).
...
Four of the five carry the same layers 2 and 4 (`border-color: Theme.glass-border`, `drop-shadow-color: Theme.shadow-popover`, `drop-shadow-blur: 22px`, `drop-shadow-offset-y: 8px`) and the same layer-3 highlight rim (a 1px `Rectangle` at `y: 1px`, inset horizontally by ~half the radius). **SelectionToolbar (surface #2) is the intentional exception on two properties:** (a) `border-color: Theme.accent` — an accent rim signals active selection mode rather than neutral glass chrome (spec §2.5 mode-context differentiation); (b) `drop-shadow-color: root.active ? Theme.shadow-popover : transparent` — the shadow is active-gated because a parked toolbar's offset-down shadow bleeds into the visible strip through the `clip: true` band (the clip only cuts what falls outside the strip; the in-strip shadow portion is not clipped), so suppressing the color to `transparent` makes femtovg's `draw_box_shadow` early-return. Do **NOT** "fix" SelectionToolbar's border or shadow back to `glass-border` / unconditional `shadow-popover` — those are load-bearing deviations. **MANUAL SYNC CONTRACT**: any change to the glass anatomy — fill token, border color, shadow blur/offset, or rim geometry — must be applied to ALL FIVE by hand (respecting SelectionToolbar's two exceptions above). There is no shared atom; the file and approximate line above are the grep anchor for each surface. The only copy-pasted *computation* is the rim geometry (`x: <half-radius>; y: 1px; width: parent.width - <inset>; height: 1px`); extracting it into a `GlassHighlightRim` atom was deliberately deferred to a follow-up issue — it is a mechanical 5-component refactor that was kept out of the UX-focused selection-polish PR.
