...
The recipe for swapping two chrome bars (the "Select" entry pill ⇄ the SelectionToolbar) with a vertical SLIDE instead of an instant `if`-gate — `Carousel.slint`'s `slide-strip`:

1. **Fixed-height `clip: true` band reserved in BOTH modes** — height pinned to `Theme.nav-pill-height` (the TALLER bar), NOT `max` of the two, so the band occupies the same space whichever bar shows and the cover-flow content below never jumps on a mode flip.
2. **The sliding elements are direct children of the plain band `Rectangle` with explicit `y:` — never children of an enclosing layout** (a layout would re-impose their position and break the slide). The SelectionToolbar is wrapped in a centering `HorizontalLayout` for its content-hug width (the "element outside a layout defaults to parent width" trap, above) — but it is that WRAPPER that carries the explicit `y` and slides; the select pill is a bare child `Rectangle`.
...
4. **Belt-and-braces input gating, NOT trusting clip hit-testing alone:** the parked bar's `TouchArea`s get `enabled:` terms AND its `accessible-action-default` arms are wrapped in an `if (root.active)` guard — both keyed to a shown/`active` flag — so a slid-away bar takes no pointer/keyboard/screen-reader input even if clip hit-testing leaks (SelectionToolbar takes an `active` flag for this; the select pill gates its `TouchArea` `enabled`).
5. **The derived shown flags must replicate the FORMER `if`-gate conditions EXACTLY**, including `!root.modal-open` and (for the entry pill) `root.items.length > 0`: `toolbar-shown: selection-mode && !modal-open`; `pill-shown: !selection-mode && !modal-open && items.length > 0`. Drop a clause and a bar reappears under a modal or over an empty library.
