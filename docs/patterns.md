...
- **The Library-screen open/close is a SNAPSHOT, and it must cover the pending flag too.** `DialogSession::open_on_library` captures the open book's runtime as `RuntimeSnapshot { view: ResolvedView, inherit_pending: bool }` BEFORE the global seed; `close_on_library` restores it — the view via `apply_resolved_view` FIRST, the `inherit_pending` flag AFTER (verbatim in both directions). The order is load-bearing for the same reason the reset-to-global ordering is: `apply_resolved_view`'s value-changing `set_*` calls CLEAR the flag, so restoring the flag first would be a no-op. The failure it prevents is not obvious: a global edit made from the Library mutated the SHARED runtime and cleared the OPEN BOOK's `inherit_pending`, so the next leave point wrote a FULL override pinning the OLD global values onto that book — "Reset to global" silently undone AND frozen stale. The category error in one clause: on the Library screen the runtime is only a global-seeded scratchpad, so a GLOBAL edit must not touch the BOOK's pending state. (Nothing on the Library screen can legitimately set the flag — the reset button exists only on the per-book dialog — which is why the restore is verbatim rather than conditional.)
