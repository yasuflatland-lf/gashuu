...
`UpdateStrategy::SelfReplace` (from `gashuu_core::update::detect_packaging`) fans out to two genuinely different mechanisms in `gashuu/src/update/install.rs`; do NOT unify them behind a single `self-replace` call. The self-replace paths cannot be exercised on a macOS dev host — they run only on real Linux-AppImage / Windows-portable installs — so they are covered by unit tests (`extract_exe_from_zip`) + the gates, and verified hands-on per-OS.
