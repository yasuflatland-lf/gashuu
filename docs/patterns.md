...
`add_books_and_refresh` chooses among four status notices by the `(added, skipped)` breakdown. That 4-way mapping was lifted out of the Slint-coupled handler into `select_add_notice(added, skipped) -> AddNotice` (a `#[derive(Debug, PartialEq)]` enum: `AlreadyInLibrary` / `NoneAddedAllSkipped` / `Added` / `AddedWithSkips`), so every corner of the 2×2 is unit-testable WITHOUT a UI. This is the same pure-decision-fn discipline as `status_notices`/`notices_content` (use-case object section, point 2 above) and `should_signal_empty` (cover-loader): keep the DECISION pure and headless; let the impure handler only map the chosen variant to the localized `dynamic.rs` call + `set_status_text`. The save-failure arm is handled separately in the handler and is deliberately NOT part of the enum (it composes onto whichever notice was chosen).

### Replicate-and-PIN a `pub(crate)` core helper that the UI crate cannot reach (reject-empty-books)

When the UI needs logic that lives in a `pub(crate)` core function unreachable across the crate boundary, replicate it as a documented byte-for-byte twin and PIN the replica with mirror-contract tests. `app.rs::derive_title(path) -> String` is a documented replica of `gashuu_core::Book::from_path`'s title derivation (directory name for a folder, file stem for a file, lossy full path as a last-resort so the title is never empty); `Book`'s derivation is `pub(crate)` to `gashuu-core` and therefore NOT importable from the `gashuu` crate, so the logic is replicated rather than a new public accessor added to core. It is used ONLY on the empty-book reject path when the source was never added (so no stored `Book::title` exists to reuse — the stored title is always preferred when present). Four `#[cfg(test)]` mirror-contract tests pin the replica to `Book`'s behavior (file stem, single-extension-drop, directory name, never-empty fallback) so a future change to `Book::from_path` that silently diverges is caught. Contrast the Slint↔Rust mirror (the scrubber entry above), where the fix was to DELETE one side and make Rust authoritative: here the two sides are in DIFFERENT crates across a `pub(crate)` wall, so a small pinned replica is the lighter alternative to widening core's public API for one fallback.
...
> Wave-1 DDD refactor: every persistence path below now routes through ONE chokepoint, `persist_view_modes(route: ViewModeRoute, …)` in `main.rs`. `write_back_view_override` and `reconcile_settings` are PRIVATE; the bullets below describe what each `ViewModeRoute` arm does, not direct call sites. See the write-direction-audit gotcha entry above for the structural rationale.

- **Write back runtime → per-book override at EVERY leave point.** `write_back_view_override(&state, &viewport, &library)` snapshots the open book's four runtime modes into its `ViewOverride` and saves the library — reached via the per-book `ViewModeRoute` arms (`LeaveViewer` for nav-away ↑ to Library, `OpenDifferentBook` for open-another at the top of `OpenBookUseCase::run`, `AppExit` for app exit, `DialogClosedOnViewer` for the Viewer settings-dialog close). WHY at every leave point: a bare keyboard toggle (D/R/C/`f`) must persist for THAT book without the user opening any dialog — exactly mirroring how PR-D made bare toggles persist into global `Settings` via the save-on-exit reconcile, and how PR-R writes the reading position back at every leave point (the per-book write sits right beside `write_back_position` at each of those sites). It is a no-op when no book is open (`open_file()` is `None`).
...
### Two sanctioned ways to dodge the Slint preferred-width binding loop on a glass pill (bulk-delete PR-4, #128; way #2 corrected in fix/ui_ux)
...
### A byte-identical JSON round-trip test is VACUOUS for a `skip_serializing_if` field unless the fixture sets it to a NON-default value (bulk-delete PR-5, #129)
...
### Combined-branch contract test: when a builder appends N independent optional lines, test the COMBINED case and PIN the ordering (bulk-delete PR-5, #129)
