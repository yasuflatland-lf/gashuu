...
This guard, the version resolution, and the migrate dispatch are now single-homed in `persist::parse_versioned_object(json, current, migrate)`, shared by `Settings::from_json` and `Library::from_json` so the two cannot drift; each caller keeps its own `from_value` mapping, error variant, and post-deserialize step. The guard order is unchanged: non-object root FIRST, then the version.

### A schema `version` from the FUTURE is a hard load error — never deserialize it, never save over it

`parse_versioned_object` returns `VersionedDocument::FromFuture { found }` when the stored `version` exceeds the binary's, and BOTH callers map that to `CoreError::FutureSchema { what, found, supported }` — `settings.json` and `library.json` alike. Two reasons, both silent-data-loss:

- **Never deserialize.** Every field is `#[serde(default)]`, so `serde_json::from_value` would happily drop every field this build does not know and return a plausible-looking value.
- **Never save over.** The survivors would be re-saved. `settings.json` kept its FUTURE version label, which permanently inoculated the file against every later migration step (each fires only when `stored < target`); `library.json` was force-stamped honestly and was therefore silently DOWNGRADED instead. Neither is recoverable after the fact.

Recovery is **quarantine-then-fresh** for both documents, reusing the pattern `library.json` already had: rename aside as `<name>.corrupt-<now_secs>` via `gashuu_core::quarantine_file` (never reads or parses the file; the caller injects the clock), surface a notice, start from defaults. Note the asymmetry this removes: before the change a failing `settings.json` was OVERWRITTEN with the defaults' canonical JSON by `repair_settings_file_if_needed`, destroying the user's bytes; that function is now inert after a quarantine (its `!path.exists()` early return).

`Settings::normalize` stamps `SETTINGS_VERSION` on save — the `version` field is a schema label OWNED BY THE BINARY, never echoed back from disk — restoring the symmetry `Library::to_json` already had via `LibraryDocument`. `normalize ∘ normalize = normalize` still holds (an assignment is a projection). `SETTINGS_VERSION` and `LIBRARY_VERSION` both stay `1`; no migration step was added.

### Enforce read-path invariants on load, but only the ones with no valid alternative

`from_json` normalizes after deserialize: `cache_size` is floored to 1 via `Settings::cache_config()`, which constructs a `CacheConfig` whose `new` enforces `capacity >= 1` (the single source of truth for that floor — `ImageCache::new` no longer clamps; it consumes a `CacheConfig` whose capacity is already guaranteed `>= 1`, so the `NonZeroUsize::new(...).unwrap()` inside it is provably safe). `recent_files.truncate(MAX_RECENT_FILES)` prevents an over-long hand-edited list from persisting forever via exit-save. **`preload_pages` is deliberately NOT clamped** — 0 is a valid "prefetch disabled" radius and not coerced downstream, so clamping it would silently override a legitimate user choice. `normalize` also STAMPS the schema `version` with `SETTINGS_VERSION` — that field is the binary's label, not user data, so it is never carried over from the loaded document (see the future-version entry above). Only a version `<= SETTINGS_VERSION` ever reaches here; a higher one is a hard load error before deserialization.
...
3. **Preserve borrow discipline EXACTLY** (the moved body carries the same RefCell-drop choreography):
   ...
   `apply_runtime_view_to_settings` call is gone — it now writes the OUTGOING book's per-book
   override via `LeavePointService::persist_with(ViewModeRoute::OpenDifferentBook, …)` near the top, and never reconciles into
...
- **ONE merge rule and its colocated inverse.** `resolve` is the single definition of the per-field fallback (`self.field.unwrap_or(global.field)` per field). `ViewOverride::differences_from(view, global)` is the inverse used for minimal write-back: it emits `Some(view.field)` only when that field differs from `global.field`, otherwise `None` (inherit). Their round-trip invariant is `differences_from(view, global).resolve(global) == view` for every resolved view and global. Because the only way to get a renderable view is `resolve`, "an unresolved view reaches the renderer" is UNREPRESENTABLE — there is no path that hands a partial `Option`-bearing value to the page pipeline.
...
STRUCTURAL ENFORCEMENT (Wave-1 DDD refactor). The audit no longer needs to chase scattered call sites: all view-mode persistence now routes through ONE chokepoint, `LeavePointService::persist(route: ViewModeRoute)` in `view_sync.rs`, and the private `stage_view_modes_to_sink` helper contains the routing `match`. Its two sink fns (`apply_runtime_view_to_settings` → global, `stage_view_override_write_back` → per-book) are reachable in production only through that chokepoint, with ONE named exception: `apply_runtime_view_to_settings` is `pub(crate)` so `DialogSession::end` can commit a Library-scope dialog's global edit at the session boundary (issue #556); `stage_view_override_write_back` stays private. The audit is therefore the routing match PLUS that single named extra caller. `ViewModeRoute` names each leave point (`DialogClosedOnLibrary` / `DialogClosedOnViewer` / `LeaveViewer` / `OpenDifferentBook` / `AppExit`), and the match routes it to the right sink; the clobber-trap rationale lives as a comment on that match. So the harness below now reduces to auditing the SINGLE routing match — a new leave point cannot reach a write fn except by adding a `ViewModeRoute` variant and a match arm, where the global-vs-per-book decision is forced into view. The cautionary lesson still holds (the trap is real and shipped once); the structure now makes the dangerous global-write sites enumerable in one place instead of grep-able across the crate.

HARNESS:

1. **Audit the ONE routing match (was: enumerate ALL write sites).** Pre-chokepoint this meant grepping both write fns and reading every call site — and the plan's enumeration missed one. Now every write goes through `LeavePointService`; read `stage_view_modes_to_sink`'s `match route` to cover all sinks. The write fns are private, so no path can bypass it. A new leave point is a new `ViewModeRoute` variant + arm, which is exactly where the global-vs-per-book routing decision is made.
2. **State the invariant explicitly, then audit every route against it.** The invariant for this feature: the GLOBAL view modes are written ONLY by (a) the global-scope UI — the Library settings dialog close (`DialogClosedOnLibrary`) — and (b) the no-book-open exit path (`AppExit`, guarded on `open_file().is_none()`); the PER-CONTEXT override is written ONLY at leave points (`stage_view_override_write_back`, the other three routes). Any route arm that reconciles into global outside (a)/(b) is the bug.
...
The `> u32::MAX → treated as 0 → migrate` rule is UNCHANGED by the future-version tier, and a refactor here must not misroute it. A malformed, missing, or `> u32::MAX` version is "unknown" and migrates from 0 — it is NOT `FromFuture`. The comparison that decides the three arms runs on the RESOLVED `u32` (`from.cmp(&current)`), after the checked conversion; a naive `from > current` written against the raw `as_u64()` would classify `u32::MAX + 1` as "from the future" and turn a crafted file into a hard load error plus a quarantine. That is the exact regression this entry exists to prevent.
...
### Per-entry 500 MB ceiling (`MAX_ENTRY_BYTES`) defends size-spoofing zip bombs — two tiers everywhere, a THIRD actual-length tier for RAR only

`MAX_ENTRY_BYTES` lives in `entry_policy.rs` (it is an archive-entry-domain property shared by BOTH `ZipSource` and `RarSource`). Open-time (both sources): skip entries whose DECLARED size > max. Read-time for `ZipSource` (and `FolderSource`): the shared `cap_or_reject(src, name, max, capacity_hint)` helper in `entry_policy.rs` does `Read::take(max+1)` then `buf.len() > max` → `EntryTooLarge` — the read-time `take` is the REAL cap (a crafted header can lie). The open-time page-membership decision (dir / non-image / macOS-metadata / oversized → `EntryClass::{Page, Skip, Ignore}`) is likewise centralized in `entry_policy.rs`'s `classify_entry`, shared by zip and rar so the rule cannot drift between them.

**The `capacity_hint` is a growth hint, and a growth hint must NEVER be derived from an attacker-controlled DECLARED size.** Zip passes `initial_capacity(entry.size())`, which clamps the declared size to `INITIAL_READ_CAPACITY` (1 MiB); folder passes `0`. The obvious-looking `min(declared, MAX_ENTRY_BYTES)` is a 500 MiB `Vec::with_capacity` from a KB-sized file that lies in its header, and under rayon prefetch (radius ≤ `MAX_PREFETCH_RADIUS` = 5 ⇒ up to 11 concurrent reads) that multiplies into GiBs of transient allocation — all of it *before* the cap can fire, because the cap is a property of the bytes actually read. The `take(max + 1)` + length check remains THE cap; `initial_capacity` is the single home of the "how much may I pre-allocate" rule. 1 MiB comfortably exceeds a real manga page, so the common read still allocates exactly once.

**`RarSource`'s read-time cap is WEAKER** — it cannot stream-cap (see the RAR bullet below), so it does not use `cap_or_reject`. It instead routes `read()`'s result through `reject_over_actual(data, name, max)`, a THIRD tier that rejects by ACTUAL length after the buffer exists, sharing `cap_or_reject`'s inclusive boundary (`len > max` rejects).
...
### RAR per-entry listing error = TRUNCATE (a distinct signal), NOT a counted skip, NOT interior-skip, NOT whole-archive hard-fail

`unrar`'s `List` iterator is NON-RESUMABLE: after any per-entry error it sets `damaged` and yields `None` forever, so (unlike `ZipSource`'s random-access `by_index` skip+count of an INTERIOR entry) RAR can only drop the TRAILING remainder. The open loop therefore does `Err => { truncated = true; break; }` — the `break` stays, but the error arm sets a flag and does **not** increment `skipped`.

**The two signals are independent and must never be conflated.** `skipped_count()` counts individual entries dropped from an otherwise COMPLETE listing; `listing_truncated()` says the listing was CUT SHORT, so an unknown number of later pages are missing. Counting the truncation as "1 skipped entry" was meaningless: the listing stopped, so the true number of lost entries is unknowable — reporting `1` was a number the code could not justify. `listing_truncated` is a defaulted trait method on `PageSource` (`false`), so `FolderSource`/`ZipSource` — whose walks are resumable — are unaffected, and the contrast with `zip.rs`'s `continue`-based INTERIOR skip stands: that is genuinely a per-entry skip and must NOT gain a truncation flag.

The open still SUCCEEDS (failing it would make every partially-readable archive unopenable) and `probe_page_count` semantics are unchanged. The user-visible signal is the `notice-listing-truncated` Fluent key (en + ja), rendered BEFORE the skipped-entries line — most severe first. NUANCE (still true): `unrar` emits a phantom `Ok("")` (empty filename) right before the terminal `Err` on a corrupt trailing header; it is filtered as neither a page nor a skip (empty name → `!has_image_ext`). This is an intentional, documented divergence from ZIP's interior skip+count.

### `RarSource`'s read-time size cap is WEAKER than `ZipSource`'s (no streaming `take`) — three tiers, and an honest residual

`unrar` 0.5.8 exposes **no** streaming, chunked, or capped in-memory read: `read()` is the only public byte-returning API and it materializes the WHOLE entry into a `Vec`. Its chunk accumulator (`ReadToVec`) sits behind the private `ProcessMode` trait, so a downstream crate cannot inject a running byte budget or abort the loop. RAR therefore has THREE tiers instead of two: declared size at open (`classify_entry`), declared `unpacked_size` re-validated at read (guarding an entry that changed between the listing and processing passes), and `reject_over_actual(data, name, max)` on the materialized buffer.

What the actual-length check BUYS: the oversized buffer is dropped immediately instead of being handed to `decode`; the page fails with the same typed `EntryTooLarge` every other source produces; and a lying header can no longer smuggle an over-ceiling payload into the decode path.

What it does NOT buy: peak RSS during `cursor.read()` is still unbounded by the entry's true size. **Do not claim parity with `ZipSource`.** This bounds the blast radius to one entry-sized allocation; it does not prevent it. `image::Limits` in `image_ops::decode` remains the final backstop.

Rejected alternative: `extract_to` a temporary file and read it back under a cap. It adds a write plus a second read on the hot rayon prefetch path, temp-file lifecycle handling on every page turn, and `extract_to` panics on a nul character in the destination path. Accepted weaker guarantee, documented at the call site.
...
### A slotted `@children` block resolves `parent`/`root` to its DECLARING component root, NOT the element it is drawn into (GlassModal dialogs)

`@children` relocates only where an element DRAWS. When a dialog `inherits` a shell that injects content via `@children` — `SettingsDialog`/`ShortcutsOverlay`/`ConfirmDialog` each `inherit GlassModal`, declaring their content `FocusScope` at the SUBCLASS root, which `GlassModal` draws inside its inner `panel` `Rectangle` via `@children` — that slotted element's `parent` in property expressions still resolves to its LEXICAL parent (the subclass's own root, which fills the window), NOT the `panel` it is drawn into. So content sized `width: parent.width; height: parent.height;` stretched to the whole window and overflowed the ~432 px panel (horizontally — Settings' `Segmented` rows ran off the right edge; vertically — ConfirmDialog's body-line list spilled below the panel). The cargo gates never catch it (see the verify-by-hand entry below). Fix: size the slotted content to the panel's KNOWN size, which the shell already owns — `GlassModal` takes `in property <length> panel-width/panel-height` and binds them onto `panel.width/height`, so each subclass reads `width: root.panel-width; height: root.panel-height;`. A child nested INSIDE the slotted element keeps `parent.*` — its lexical parent IS that element, resolved correctly. Same family as the two `root`/`parent` entries above: there a component root is BLIND to `parent`; here the slotted child CAN read `parent`, but it points at the declaring root, not the draw site. (The dialogs used to nest this content INSIDE the panel, where `parent.*` resolved to the panel; the GlassModal extraction moved it to the subclass root, silently changing what `parent` meant.)
...
- **Tab-enabling a non-focusable button atom WITHOUT editing the atom — the shared `FocusButton` wrapper (issue 127):** When a TouchArea-based button atom (`DangerButton`/`PrimaryButton`) must become a Tab stop inside a multi-stop modal, wrap it in the shared `FocusButton` — exported from `components/FocusButton.slint` and imported by BOTH `ConfirmDialog.slint` and `UpdateAvailableDialog.slint` (the same wrapper idiom `NavBar` uses in-file for its `SearchField` and `Stepper` for its `StepButton`). It is a `Rectangle` around an inner `scope := FocusScope`, with `forward-focus: scope;` (so an outside `<id>.focus()` still lands on the scope) and `out property <bool> has-focus: scope.has-focus;` (so a modal's Tab rotation can still read which stop holds focus). The wrapper sizes to its slotted child via `@children` and exposes a `callback pressed` plus `in property <bool> enabled: true`. Key model: Space accepts and fires `pressed`; a catch-all `return reject` rejects every other key (including Tab, Backtab, Return, and Esc) so they bubble up to the ancestor modal `FocusScope`. **`enabled` gates only the `pressed()` emission — never the inner `FocusScope`'s own `enabled`**: that built-in refuses focus by click, by Tab AND programmatically, so binding it would drop a disabled-but-focused button out of the modal rotation (it is also why the wrapper can NOT inherit `FocusScope` — the base already declares `enabled`, and a component cannot re-declare a property of its base). A Space press on a disabled wrapper is still swallowed (`return accept`) so the key never leaks to the ancestor `fs` (update dialog in-progress guard, issue 481). This partition is compositionally safe: because the button scope rejects Return, the ancestor's "Return ⇒ cancel" binding fires even when the confirm button holds focus, so the destructive action is structurally unreachable by Enter (ConfirmDialog, issue 127).
...
### Glass chrome: `GlassModal` single-sources the THREE dialog panels; only the TWO pill surfaces (NavBar, SelectionToolbar) still hand-wire (GlassModal extraction)

The glass panel chrome — scrim + the 4-layer fake-glass object (top-sheen FILL gradient layer 1 + 1px `glass-border` rim + 1px `glass-highlight` top rim + ONE drop-shadow) — is now SINGLE-SOURCED by `components/GlassModal.slint` for the three modal dialogs: `SettingsDialog`, `ShortcutsOverlay`, and `ConfirmDialog` each `inherit GlassModal` and own NO glass markup of their own (they set `panel-width`/`panel-height`, wire `backdrop-clicked`, and slot their content as `@children`; see the slotted-`@children` sizing entry). A glass-anatomy change to the DIALOGS is now a one-file edit in `GlassModal`. The TWO pill chrome surfaces still hand-wire the 4-layer recipe — the pill variant differs from the panel (solid `glass-fill` layer 1 instead of the sheen gradient; the highlight rim inset by `nav-pill-height / 2` because a pill's radius caps at half its height) — with NO shared atom:

1. **NavBar** (`components/NavBar.slint`) — `background: Theme.glass-fill`; neutral solid-fill pill.
2. **SelectionToolbar** (`components/SelectionToolbar.slint`) — the same recipe as NavBar with two intentional departures (see the note below).

Both pill surfaces carry the same layers 2 and 4 (`border-color: Theme.glass-border`, `drop-shadow-color: Theme.shadow-popover`, `drop-shadow-blur: 22px`, `drop-shadow-offset-y: 8px`) and the same layer-3 highlight rim (a 1px `Rectangle` at `y: 1px`, inset horizontally by half the pill height (`nav-pill-height / 2`) — a pill's corner radius caps at half its height, so this keeps the rim inside the rounded corners). **SelectionToolbar is the intentional exception on two properties:** (a) `border-color: Theme.accent` — an accent rim signals active selection mode rather than neutral glass chrome (spec §2.5 mode-context differentiation); (b) `drop-shadow-color: root.active ? Theme.shadow-popover : transparent` — the shadow is active-gated because a parked toolbar's offset-down shadow bleeds into the visible strip through the `clip: true` band (the clip only cuts what falls outside the strip; the in-strip shadow portion is not clipped), so suppressing the color to `transparent` makes femtovg's `draw_box_shadow` early-return (see the "`clip: true` does not clip a child's box shadow" entry below for the femtovg scissor/shadow mechanism behind this gate). Do **NOT** "fix" SelectionToolbar's border or shadow back to `glass-border` / unconditional `shadow-popover` — those are load-bearing deviations. **MANUAL SYNC CONTRACT (pill surfaces only)**: any change to the pill glass anatomy — fill token, border color, shadow blur/offset, or rim geometry — must be applied to BOTH NavBar and SelectionToolbar by hand (respecting SelectionToolbar's two exceptions above). The three DIALOG panels no longer participate — they inherit `GlassModal`, which single-sources the panel recipe. Extracting the shared pill rim (`x: Theme.nav-pill-height / 2; y: 1px; width: parent.width - Theme.nav-pill-height; height: 1px`) into a `GlassHighlightRim` atom remains deferred — now a mechanical 2-component refactor.
...
A parent cannot reach an `id` inside an `if`-gated child (the `root` vs `parent` / `focus-control()` encapsulation limit), so it cannot call `fs.focus()` on a still-mounted modal after a stacked overlay closes. The seam now covers all three stacked surfaces — `SettingsDialog`, `ShortcutsOverlay`, and `ConfirmDialog`: the parent holds a PRIVATE focus-epoch property and a public focus function for each; the child declares `in property <int> focus-epoch` bound to that property plus a root-level `changed focus-epoch => { fs.focus(); }`. Bumping the int from Rust drives the child's own `FocusScope` so its keys are live again on return. This works BECAUSE Slint `changed` handlers do NOT fire on the initial binding/mount — only on subsequent value changes — so the OPEN sequence stays owned entirely by `init` (the `focus-control()`/`init`-fires-each-open entry) and the epoch handler fires ONLY on a genuine re-focus request. The topmost update dialog therefore restores focus stack-aware in paint order: ConfirmDialog → ShortcutsOverlay → SettingsDialog → carousel/pages. It must never focus a screen while a modal survives, because the screen FocusScopes reject every key under `any-modal-open`. Same class as the `focus-pages()`/`focus-carousel()` seam — focus is an invariant the code restores explicitly at every transition.
...
**The second mechanism: BOUNDARY CONTAINMENT, for a modal whose stop set is VARIABLE.** `SettingsDialog` cannot enumerate its stops at all: it has drill-in screens 0..5 whose controls live inside `if` subtrees, and an id inside an `if` subtree is unreachable from the ancestor `fs` — so "rotate among the stops" is not implementable there, no matter how the rotation is written. Contain at the EDGES instead: declare zero-size sentinel `FocusScope`s at the dialog subtree's document-order boundaries — one immediately BEFORE the scope, one as the scope's LAST child — each redirecting focus back into the dialog the moment window tab-nav lands on it (`changed has-focus => { if (self.has-focus) { fs.focus(); } }`). Window navigation still runs; it simply cannot escape past either edge.

Record the ACCEPTED DEGRADATION so nobody "fixes" it later: the forward wrap transits `fs` (one extra Tab to reach the first control), and Backtab at the first stop parks on `fs` — where Esc and the two-stage close still work — instead of wrapping to the last stop. **Contained is the requirement; perfect rotation is not.**

Record why a blanket "swallow Tab at `fs`" is WRONG for this dialog specifically: every in-dialog atom REJECTS Tab, so every press bubbles up to `fs` BEFORE the window-level fallback runs. Accepting there would kill all Tab movement inside the dialog — the exact "old catch-all" failure this entry already documents, just one level up.

The decision rule in one line: **enumerate when the stop set is fixed; sentinel when it is variable.**

### A component's OWN callback name can shadow the `EventResult` enum — bare `accept`/`reject` in `key-pressed` need the `EventResult.` qualifier when the component also declares a same-named callback (UpdateAvailableDialog, auto-update epic)

`key-pressed` handlers conventionally `return accept;` / `return reject;` — the bare identifiers resolve to the `EventResult` enum's `accept`/`reject` variants, exactly as `ConfirmDialog` and `ShortcutsOverlay` do (see the two modal entries above). `UpdateAvailableDialog` breaks that convention: it is a THREE-choice dialog whose pinned public API (auto-update epic) names one of its own callbacks `callback accept();` (fired by the "Update now" button). Inside that component's `fs.key-pressed` handler, the bare identifier `accept` is now ambiguous between the enum variant and the component's own callback — Slint resolves it to the CALLBACK, and a bare callback reference used where a value is expected fails to compile with "Callback must be called". The fix is to fully qualify every `EventResult` return in that handler: `return EventResult.accept;` / `return EventResult.reject;`. This is scoped to the component that owns the colliding name — the shared `FocusButton` wrapper (`components/FocusButton.slint`) declares no `accept`/`reject` callback, so its own `key-pressed` handler keeps the bare `return accept;` / `return reject;` form unambiguously. Rule of thumb: before naming a callback `accept`, `reject`, or any other `EventResult` variant name, check whether that component's own `key-pressed` (or a handler on an element it lexically contains) still uses the bare form — if so, either qualify with `EventResult.` throughout or pick a non-colliding callback name (`ConfirmDialog`'s `confirm`/`cancel` never collide, which is why its catch-all `accept` in the entry above needed no qualifier).
...
### Runtime state is the SINGLE source of truth for the four display modes; `Settings` mirrors them ONLY via `apply_runtime_view_to_settings`, just before each save (issue #32)

`ViewerState` owns `reading_direction`/`spread_mode`/`cover_mode`; `ViewportState` owns `fit_mode`. `apply_runtime_view_to_settings(&ViewerState, &ViewportState, &mut Settings)` (a pure, `pub(crate)` fn in `view_sync.rs`, reached via `LeavePointService` and — since issue #556 — from `DialogSession::end`) copies those four into `Settings` before a GLOBAL save — the no-book-open exit path and the Library-screen settings-dialog close. (The open-time save in `app::OpenBookUseCase::run` does NOT reconcile — that was the clobber bug; see the write-direction-audit gotcha.) Mode-mutation sites (D/R/C/`f` keys + the dialog setters) now ONLY mutate runtime state + `refresh`; the ~9 per-mutation `settings.borrow_mut().X = …` mirror lines are GONE, killing the "a new mutation site forgets to mirror → setting silently not persisted" bug class (neither types nor tests caught it before). EXCEPTION: `cache_size`/`preload_pages`/`track_recent_files` keep `Settings` as their source (one-way `Settings → ViewerState` via `set_cache_config` — see that bullet above); they are NOT reconciled back. `on_open_settings` reads the dialog's initial mode values from the RUNTIME (`state`/`viewport`), never `Settings`, so a lagging mirror can't make the dialog show a stale value.
...
> Wave-1 DDD refactor: every persistence path below now routes through ONE chokepoint, `LeavePointService::persist(route: ViewModeRoute)` in `view_sync.rs`. `stage_view_override_write_back` is PRIVATE and `apply_runtime_view_to_settings` is `pub(crate)` (its only caller outside the chokepoint is `DialogSession::end`); the bullets below describe what each `ViewModeRoute` arm does, not direct call sites. See the write-direction-audit gotcha entry above for the structural rationale.

- **Write back runtime → per-book override at EVERY leave point.** `stage_view_override_write_back(&state, &viewport, &dialog_session, &settings, &library)` snapshots the open book's resolved runtime modes and writes their value diff against the SAME global `Settings` instance resolution uses: a differing field becomes `Some(runtime)`, while a matching field stays `None` (inherit). `LeavePointService` saves the staged library — reached via the per-book `ViewModeRoute` arms (`LeaveViewer` for nav-away ↑ to Library, `OpenDifferentBook` for open-another at the top of `OpenBookUseCase::run`, `AppExit` for app exit, `DialogClosedOnViewer` for the Viewer settings-dialog close). WHY at every leave point: a bare keyboard toggle (D/R/C/`f`) must persist for THAT book without the user opening any dialog — exactly mirroring how bare toggles persist into global `Settings` via the save-on-exit reconcile, and how the reading position is written back at every leave point (the per-book write sits right beside `stage_position_write_back` in the chokepoint). It is a no-op when no book is open (`open_file()` is `None`); pending reset intent still short-circuits to `ViewOverride::none()` before the diff.
- **Gate the GLOBAL reconcile on "no book open."** The `AppExit` route writes the per-book override FIRST, then runs `apply_runtime_view_to_settings` only `if state.borrow().open_file().is_none()` — otherwise the open book's per-book modes (just written to its override) would clobber the global defaults. This is the exit half of the write-direction invariant in the gotcha entry above.
- **The SAME settings dialog edits different targets by SCREEN.** There is ONE `SettingsDialog`; `ui.get_screen()` selects the `ViewModeRoute` on close — `0` (Library) → `DialogClosedOnLibrary` → reconcile into GLOBAL `Settings`; `1` (Viewer) → `DialogClosedOnViewer` → `stage_view_override_write_back` into the current book. Seeding the dialog when opened over the Library requires mirroring GLOBAL → runtime FIRST (`apply_global_view_to_runtime(&settings, &state, &viewport)`), so the runtime carries the global defaults while that dialog is open; closing it on the Library screen runs the inverse reconcile. (On the Viewer screen the runtime already holds the open book's resolved modes, so no pre-seed is needed.)
- **The dialog session routes by its RECORDED opening scope, and owns the pending intent too.** `DialogSession::open(scope, ...)` records `DialogScope::Library` or `Viewer`; Library scope captures the open book's runtime as `RuntimeSnapshot { view: ResolvedView, pending_inherit: Option<ResolvedView> }` BEFORE the global seed, while Viewer scope leaves the runtime untouched. Idempotent `end()` runs three steps in a FIXED order: (1) if the ENDING session's recorded scope is `Library`, reconcile the live runtime into the global `Settings` via `apply_runtime_view_to_settings` — so the dialog's global edit is committed wherever the ending happens; (2) restore the snapshot — the view via `apply_resolved_view` FIRST, `pending_inherit` AFTER (verbatim in both directions), so the restored view is the one the comparison observes; (3) clear both scope and snapshot. **Reconcile-before-restore is load-bearing:** swapping (1) and (2) writes the OPEN BOOK's view into the global defaults — the exact ADR-0007 clobber the session boundary exists to prevent (issue #556). A `Viewer`-scope session and an `end()` with no active session stay pure no-ops for `Settings`, and `end` mutates `Settings` in memory only — the save stays at the existing save points. `inherit_pending(current)` derives the guard from `pending_inherit == Some(current)`: viewer-mode changes, viewport fit changes, and future setters all disable it by changing runtime state, without any setter clearing a flag; returning to exactly the reset-installed view re-arms it. Persistence uses the recorded scope even if the screen changes; keyboard focus uses the current screen. `end()` runs from both close-handler branches, exit persistence before `AppExit`, and `on_open_finalize` before `apply_probed` — so no leave point can write the global-seeded scratchpad into the open book, AND (via step (1)) no leave point silently discards a Library-scope global edit either. The Library close branch still runs its own `DialogClosedOnLibrary` route before `end()`, so that branch writes the same value twice; the doubled write is idempotent because the runtime has not moved between them (`end`'s restore happens after both), and keeping the route intact leaves the `stage_view_modes_to_sink` routing table untouched. A GLOBAL edit must not touch the BOOK's pending state; nothing on the Library screen can legitimately set that intent.
...
TRADE-OFF worth stating: write-back is a VALUE DIFF against the current global, not a record of which controls the user touched. A book left at a value equal to global stores `None` for that field and therefore FOLLOWS a later global change, even if the user had explicitly selected the matching value. A differing value remains `Some` and survives later global changes. This restores active inheritance without per-field dirty flags; see ADR-0007's 2026-08-16 amendment.
...
first member supplies the title, and `last_opened` is repointed to the survivor. The I/O remains
outside the aggregate's pure `normalize()` hook. Session scope still matters: a duplicate created
after startup is not healed immediately; it converges on the next load. Related: the
canonical-keying discipline below ("Key `Library` by the CANONICAL path") and the
`add`-returns-stored-value entry (#82) above; the emptiness rule that runs at the SAME add boundary
is ADR-0009.
...
`ThumbnailCache::put` QOI-encodes to memory and delegates to `write_atomic`, which creates a random same-directory `NamedTempFile` (`.tmpXXXXXX`), syncs it, and persists it over `<dir>/{key}.qoi`. The rename is atomic on POSIX, so a concurrent `get` (e.g. the background cover rayon fill racing a read) never observes a torn QOI — it sees either the old file or the complete new one. Random temp names also keep concurrent same-key writers from clobbering a shared deterministic temp path; the last successful rename wins with a complete file. A process interrupted before rename may strand a `.tmp*` file, which the cache ownership and pruning rules recognize. Correspondingly, `get` reads only the current `.qoi` path and treats every missing/unreadable/corrupt file as `None` (a cache miss), never an error, and never panics.
...
UPDATE — on the OPEN path the hook is now "remove + **close** + notify", not just "remove + notify". `OpenBookUseCase`'s empty-book arm calls `ViewerState::close()` after `remove_empty_book` and before returning the variant. PINNED post-rejection state: `open_file()` is `None`, `page_count()` is `0`, and NO source is mounted. The viewer is CLOSED rather than restored to the previously open book because the outgoing book's position and per-book override were already persisted by the leave-point write at the top of the apply half — nothing is lost — and this mirrors the bulk-remove use case's close-don't-restore precedent. The mounted-zero-page alternative is what the wave removed: it showed the just-REMOVED book in the title bar, let the Library's Down key pass its only guard (`open_file().is_none()`) into a 0-page Viewer, and made the `AppExit` route treat a phantom book as open.

### "Zero pages / blank covers" can be an UNMOUNTED VOLUME, not a decode regression — check `library.json` paths + the mount table FIRST (PR #334)

A report of "I select a book and the Viewer shows 0 pages" together with "the carousel covers are all the failed-cover placeholder" reads like a decode/cover regression but was an environmental red herring: every `library.json` book pointed at `/Volumes/share/…` (an SMB share) that was not mounted, so the files did not exist — the archive open returns `NotFound` and the cover load fails the same way. Before chasing a code bug in the open/cover path, dump the stored paths and check existence (`python3 -c "import json,os; [print(os.path.exists(b['path']), b['path']) for b in json.load(open(LIB))['books']]"`) and `ls /Volumes` / `mount`. No decode-error lines in the log + every path missing == environmental, not a regression. This is the missing-drive complement to the "empty vs unreadable" rule above (a missing book is NOT removed); the graceful handling for it is the positive-gate / `viewer-open-inaccessible` entry below.
...
The live open path is the `on_carousel_open` / `on_carousel_continue_reading` handlers, which route through the shared `open_and_enter` helper; that helper does the identical `match outcome { Error → set status, Success → refresh + append notices }` ONCE via the extracted `finalize_open`, rather than each handler open-coding it. Extract `fn finalize_open(ui, state, viewport, localizer, outcome)` in `main.rs` — NOT in `app.rs` — so `app.rs` stays free of `crate::i18n`. The helper belongs to the layer that owns both the outcome type AND the locale.

### Gate a screen transition on the POSITIVE outcome (`Success`), not "NOT this one failure" — and an unreachable-file open stays on the Library (PR #334)

`open_and_enter` (`handlers/library.rs`, the carousel-open / bookmark-jump sites) decided whether to enter the Viewer with `enter_viewer = !matches!(outcome, OpenOutcome::EmptyBookRejected { .. })` — "enter unless it was an empty-book removal." That NEGATIVE gate let `OpenOutcome::Error(_)` through, so opening a book whose file had moved (or whose volume was unmounted) still switched to a blank 0-page Viewer; the localized error from `finalize_open` was written to the Library status strip the user never saw. Gate on the POSITIVE case: `enter_viewer = matches!(outcome, OpenOutcome::Success(..))`. A "not this one bad case" guard is fragile — every NEW outcome variant defaults into the success/enter branch. On a failed open the Library now also explains itself: when `Error(_) && !path.exists()` (the file is gone / its volume isn't mounted), the status is replaced with the localized `viewer-open-inaccessible` message NAMING the book via `book_display_title(lib, path)` (the stored-`Book::title`-preferred / `gashuu_core::display_title`-fallback rule, single-homed in `open_book.rs` and reused by `remove_empty_book_with`); a failure with the file STILL present (e.g. a corrupt archive) keeps `finalize_open`'s detailed error. Gate-invisible — verified by running, not the cargo gates (see the screencapture-harness entry). Extends the "`OpenOutcome` pattern" / "`finalize_open` helper" entries above with the screen-transition decision they did not cover.

Two corrections since the off-thread single open landed: the `!path.exists()` decision that selects the `viewer-open-inaccessible` message now comes from the OFF-THREAD probe result (`OpenProbeOutcome::Failed { path_exists }`, stat'd on the rayon worker) rather than a live `path.exists()` stat on the UI thread — a stat on an unmounted network volume is exactly the call that could block the event loop. The positive `enter_viewer` gate itself is UNCHANGED; it simply moved, with the rest of the apply half, into the `open-finalize` handler.
...
The carousel `FocusScope.key-pressed` has `if (root.modal-open) { return reject; }` at the top, but that guard only applies to keyboard events dispatched through that `FocusScope`. A `scroll-event` on a `TouchArea` is NOT routed through `FocusScope`; it reaches the `TouchArea`'s own `scroll-event` handler directly. Without an explicit `if (root.modal-open) { return reject; }` in the scroll handler, a two-finger horizontal swipe over an open modal (whose backdrop `TouchArea` has no `scroll-event` and so doesn't absorb it) will fall through to the underlying swipe handler and move the carousel under the modal. Fix: repeat the `modal-open` guard as the **first** statement in every `scroll-event` handler that must be modal-aware. In the viewer, `PageView` now takes an `in property <bool> modal-open` (bound from `ViewerWindow` to `show-settings || show-shortcuts || show-confirm-delete || show-update-available`) and gates BOTH the `scroll-event` first line AND the `ScaleRotateGestureHandler.enabled` on it — backdrops absorb pointer clicks but not scroll/gesture events.

### All four open flags belong in every modal guard: `show-settings`, `show-shortcuts`, `show-confirm-delete`, and `show-update-available`

`ViewerWindow`'s `modal-open` property is `show-settings || show-shortcuts || show-confirm-delete || show-update-available`. Any direct guard in `ViewerWindow` that lists fewer than all four open flags silently lets input through under the omitted dialog. Both the `tapped` (tap-to-turn) and `page-swipe` (swipe-to-turn) guards in `ViewerWindow.slint` therefore require `!root.show-settings && !root.show-shortcuts && !root.show-confirm-delete && !root.show-update-available`. RULE: every modal-aware input guard must cover all four open flags.

### Absorbing the macOS trackpad momentum phase: idle-gap vs fixed-cooldown vs release-latch — pick by whether the action should REPEAT

macOS's momentum-phase scroll stream keeps delivering `delta-x` events for a variable 0.5–1.5 s after the fingers lift, decaying with flick speed. A discrete swipe gesture (accumulate `delta-x`, cross a threshold, fire once) must guard against that tail so ONE flick does not fire many times. Three models, from strictest to loosest:

- **Idle-gap re-armable Timer → "exactly one action per gesture, NEVER repeat".** Declare `idle-timer := Timer { interval: idle-gap; running: armed; triggered => { armed = false; } }` and call `idle-timer.restart()` inside `scroll-event` on every event you swallow while `armed`. The guard clears only once the stream is quiet for the whole idle gap, so the entire momentum tail (however long) is absorbed and can never re-fire. But it also blocks a deliberate re-flick until ALL momentum dies — so it CANNOT do consecutive turns. Use when a repeat is a bug.
- **Fixed-duration cooldown → repeats, but DOUBLE-FIRES on a fast flick.** Set `cooling = true` on fire, swallow while `cooling`, let a non-restarted `Timer` clear it after N ms. After it clears, ongoing/re-flicked input re-accumulates. The trap: a fast flick's momentum OUTLASTS any fixed N (momentum runs 0.5–1.5 s), so the still-arriving tail re-crosses the threshold and fires an unwanted extra step — verified in the viewer: one flick turned two pages. `Carousel.slint` tolerates this (an extra book-step is cheap); do NOT use it where a skip loses content.
- **Release-latch + velocity-spike → consecutive turns WITHOUT double-firing (what `PageView.slint` uses).** On fire set `latched = true` and reset `accum`. While `latched`, swallow every event and clear the latch only on a clear sign of a NEW swipe: **(a) a lull** — per-event `|delta-x|` drops below a small `release-delta` (≈6 px = momentum spent / finger lifted), OR **(b) a velocity SPIKE** — `|delta-x|` jumps back UP by `spike-jump` (≈25 px) above the *previous* event, which a monotonically-decaying momentum tail never does but a deliberate re-flick does. A re-armable idle `Timer` is the fallback for a lift with no momentum. Why the spike test is REQUIRED, not just the lull: a pure lull-only latch forces an impossible trade — a fast re-flick *during* momentum has large deltas that keep it latched (feels dull), and raising `release-delta` to release sooner lets the decaying tail itself re-cross the threshold and double-fire (verified as "two pages / two spreads per flick" in spread mode). Keeping `release-delta` SMALL swallows the whole tail (no double-fire) while the spike test catches the re-flick by its rising magnitude (sharp consecutive). Tuning: `spike-jump` UP = duller consecutive; `release-delta` DOWN if a single flick ever double-fires. A fixed timer can do neither — it only sees elapsed time, not momentum magnitude.

`Timer.restart()` is safe to call on a running timer (vendored `i-slint-core-1.16.1/timers.rs:137`: it resets the elapsed via `start_or_restart_timer`).
...
### Self-update replaces the running binary DIFFERENTLY per packaging (AppImage vs Windows) — and macOS/deb never self-replace

`UpdateStrategy::SelfReplace` (from `gashuu_core::update::detect_packaging`) fans out to two genuinely different mechanisms in `gashuu/src/update/install.rs`; do NOT unify them behind a single `self-replace` call. The self-replace paths cannot be exercised on a macOS dev host — they run only on real Linux-AppImage / Windows-portable installs — so they are covered by unit tests (`extract_exe_from_zip`) + the gates, and verified hands-on per-OS using the procedure in [toolchain.md](toolchain.md), "Post-release hands-on checklist: self-update + relaunch".

- **Linux AppImage: replace `$APPIMAGE`, NOT `current_exe()`.** A running AppImage's `current_exe()` points *inside* the read-only mounted squashfs (`/tmp/.mount_*`), which cannot be written — `self-replace` would fail. The file the user actually launched is `$APPIMAGE`. Replace *that*: copy the verified new `.AppImage` to a sibling `<$APPIMAGE>.new` (same directory, so the final step is an atomic same-filesystem `rename`; a cross-device rename from the Downloads dir would fail with `EXDEV`), `chmod 0755`, then `rename` over `$APPIMAGE`. `chmod` uses `std::os::unix::fs::PermissionsExt`, so the AppImage arm is `#[cfg(unix)]` with a non-Unix stub to keep the crate cross-compiling.
- **Windows portable: use the `self-replace` crate.** Windows refuses to overwrite a running `.exe` directly; the crate performs the rename-self dance (move the running exe aside, drop the new one in place). The relaunch target is `current_exe()` (now the new binary). The crate is `self-replace` on crates.io but imports as `self_replace` — the plan's `self_replace = "1"` dependency line is wrong; the manifest key must be `self-replace`.
- **Verify the ZIP, THEN extract.** `SHA256SUMS` lists the *release asset* — for Windows that is the `.zip`, not the inner `.exe`. `download_and_verify` checks the downloaded zip's hash; only then is the `.exe` extracted from the *already-verified* archive (`extract_exe_from_zip`, which reduces the entry name to its final path component so a crafted zip can't escape the out-dir). Never hash the extracted exe against `SHA256SUMS` — it isn't listed there.
- **macOS `.app` and deb are deliberately NOT self-replaced.** macOS is `ManualInstall` (unsigned + Gatekeeper quarantine would flag an in-place-swapped bundle as "damaged"); deb is `ExternalInstall` (dpkg owns `/usr/bin/gashuu`). This is a strategy decision in core, unit-tested in `packaging.rs::strategy_mapping` — the UI never second-guesses it.

The whole pipeline runs off the UI thread (`rayon::spawn`) and marshals back with `slint::invoke_from_event_loop`; on success it sets a localized "Restarting…" status, then `slint::Timer::single_shot` lets it paint for ~900ms. The timer invokes the `persist-before-relaunch` bridge to run the SAME shared exit persistence as a normal quit before `relaunch_and_exit` spawns the new binary and `exit(0)`s, because `exit(0)` bypasses everything after `ui.run()`. On ANY failure it falls back to `opener::open(RELEASES_PAGE_URL)` so the user is never stranded on a broken self-update.
