...
**`ThumbnailCache::clear` is best-effort, mirroring `prune`** (`thumbnail_cache.rs:142`). It returns a `ClearCacheReport { removed_files: usize, removed_bytes: u64 }` (`thumbnail_cache.rs:257`) — NOT a `Result`. It is non-recursive (one `read_dir`, no descent), uses `symlink_metadata` so it never follows a symlink, and unlinks only files whose names match `is_owned_cache_file_name` (`thumbnail_cache.rs:249`): `*.png` thumbnails and `.*.tmp` interrupted writes. A missing or unreadable directory short-circuits to a zero report (`let Ok(entries) = read_dir(...) else { return report }`), exactly like `prune`. The handler (`handlers/settings.rs:322`) calls `clear()` ONLY inside the `Ok` arm of `ThumbnailCache::new()`; a `new()` failure (e.g. `NoDataDir`) shows the failure status and never touches disk.
