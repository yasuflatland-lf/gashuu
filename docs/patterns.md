...
- **The dialog session routes by its RECORDED opening scope, and the Library snapshot covers the pending flag too.** `DialogSession::open(scope, ...)` records `DialogScope::Library` or `Viewer`; Library scope captures the open book's runtime as `RuntimeSnapshot { view: ResolvedView, inherit_pending: bool }` BEFORE the global seed, while Viewer scope leaves the runtime untouched. Idempotent `end()` restores the view via `apply_resolved_view` FIRST, the `inherit_pending` flag AFTER (verbatim in both directions), then clears both scope and snapshot. The order is load-bearing because `apply_resolved_view`'s value-changing `set_*` calls CLEAR the flag. Persistence uses the recorded scope even if the screen changes; keyboard focus uses the current screen. `end()` runs from both close-handler branches, exit persistence before `AppExit`, and `on_open_finalize` before `apply_probed`, so no leave point can write the global-seeded scratchpad into the open book. A GLOBAL edit must not touch the BOOK's pending state; nothing on the Library screen can legitimately set that flag.
