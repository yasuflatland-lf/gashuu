...
Confirmed ACCEPTED by the pinned Slint 1.16.1 (future work need not re-verify): `Math.clamp(x, lo, hi)`, `overflow: elide` on `Text`, `color.with-alpha(f)`, 8-digit `#rrggbbaa` hex, and `@linear-gradient(deg, stop% … )`. NOT supported: per-`Repeater`-item z-order is not settable in Slint 1.x — layer a focused item via opacity/size/accent-ring, not z. A shared PRIVATE (non-exported) sub-component (`component Foo inherits Rectangle { in property … }`) cleanly de-duplicates repeated markup WITHOUT touching any exported struct/component contract, and works both absolutely positioned and as a layout child (PR-C used one — `ProgressBar` — for the per-cover and focused-meta progress bars in `Carousel.slint`).
...
### `0 = unknown` sentinel for `usize` counts — guard callers, because a SUCCESSFUL open can legitimately yield 0 (PR-La)

Both `Book::last_page` and `Book::page_count` use `0` as the not-yet-known sentinel. The setter `Library::set_page_count` carries `debug_assert!(count > 0, "0 is the unknown sentinel")` so a bogus 0 is caught in debug/test. THE TRAP: `ViewerState::open_path` returns `Ok(())` even for a source that opens with ZERO pages (empty folder, or an archive whose every entry was zip-slip/oversized-skipped), so `page_count()` is legitimately `0` after a *successful* open. The page-count back-fill on the open path therefore MUST guard `if n > 0` before calling the setter — without the guard, a legitimate zero-page open trips the setter's `debug_assert` (a debug/test panic on valid input). The carousel reader side relies on the same sentinel: `ReadingProgress::fraction` guards `total == 0 → 0.0`, so a never-opened book reads as unread.

### Enforce one invariant at two layers: headless `debug_assert!` in core + `tracing::warn!` at the UI call site (PR-60)

The same rule is expressed twice, at different layers and with different severity. In core (no `tracing` — core stays logging-free), `Library::set_page_count` carries `debug_assert!(count > 0)`: a dev/test build trips a loud assertion; a release build pays nothing. At the UI call site, the caller MUST short-circuit with `if page_count > 0 { library.set_page_count(…) }` BEFORE calling, so a legitimate zero-page open NEVER reaches the assert in any build. An "unreachable" UI branch (the path where `set_page_count` is somehow reached with 0 despite the guard) gets a `tracing::warn!` — UI-layer only, because `tracing` is forbidden in core — so a future invariant break is debuggable in a log-enabled run instead of silently wrong. Pattern: core asserts the contract; the UI caller enforces the precondition and warns if the envelope slips.
