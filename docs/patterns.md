...
3. **Preserve borrow discipline EXACTLY** (the moved body carries the same RefCell-drop choreography):
   ...
   override via `persist_leave_point(ViewModeRoute::OpenDifferentBook, …)` near the top, and never reconciles into
...
STRUCTURAL ENFORCEMENT (Wave-1 DDD refactor). The audit no longer needs to chase scattered call sites: all view-mode persistence now routes through ONE chokepoint, `persist_leave_point(route: ViewModeRoute, …)` in `view_sync.rs`, and the private `stage_view_modes_to_sink` helper contains the routing `match`. Its two sink fns (`apply_runtime_view_to_settings` → global, `stage_view_override_write_back` → per-book) are PRIVATE and reachable in production only through that chokepoint. `ViewModeRoute` names each leave point (`DialogClosedOnLibrary` / `DialogClosedOnViewer` / `LeaveViewer` / `OpenDifferentBook` / `AppExit`), and the match routes it to the right sink; the clobber-trap rationale lives as a comment on that match. So the harness below now reduces to auditing the SINGLE routing match — a new leave point cannot reach a write fn except by adding a `ViewModeRoute` variant and a match arm, where the global-vs-per-book decision is forced into view. The cautionary lesson still holds (the trap is real and shipped once); the structure now makes the dangerous global-write sites enumerable in one place instead of grep-able across the crate.

HARNESS:

1. **Audit the ONE routing match (was: enumerate ALL write sites).** Pre-chokepoint this meant grepping both write fns and reading every call site — and the plan's enumeration missed one. Now every write goes through `persist_leave_point`; read `stage_view_modes_to_sink`'s `match route` to cover all sinks. The write fns are private, so no path can bypass it. A new leave point is a new `ViewModeRoute` variant + arm, which is exactly where the global-vs-per-book routing decision is made.
...
`ViewerState` owns `reading_direction`/`spread_mode`/`cover_mode`; `ViewportState` owns `fit_mode`. `apply_runtime_view_to_settings(&ViewerState, &ViewportState, &mut Settings)` (a pure, PRIVATE fn in `view_sync.rs`, reached only via `persist_leave_point`) copies those four into `Settings` before a GLOBAL save — the no-book-open exit path and the Library-screen settings-dialog close. (The open-time save in `app::OpenBookUseCase::run` does NOT reconcile — that was the clobber bug; see the write-direction-audit gotcha.) Mode-mutation sites (D/R/C/`f` keys + the dialog setters) now ONLY mutate runtime state + `refresh`; the ~9 per-mutation `settings.borrow_mut().X = …` mirror lines are GONE, killing the "a new mutation site forgets to mirror → setting silently not persisted" bug class (neither types nor tests caught it before). EXCEPTION: `cache_size`/`preload_pages`/`track_recent_files` keep `Settings` as their source (one-way `Settings → ViewerState` via `set_cache_config` — see that bullet above); they are NOT reconciled back. `on_open_settings` reads the dialog's initial mode values from the RUNTIME (`state`/`viewport`), never `Settings`, so a lagging mirror can't make the dialog show a stale value.
