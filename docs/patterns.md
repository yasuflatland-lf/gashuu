...
- **macOS `.app` and deb are deliberately NOT self-replaced.** macOS is `RevealDownload` (unsigned + Gatekeeper quarantine would flag an in-place-swapped bundle as "damaged"); deb is `OpenReleasePage` (dpkg owns `/usr/bin/gashuu`). This is a strategy decision in core, unit-tested in `packaging.rs::strategy_mapping` — the UI never second-guesses it.
