...
A double-click on a `TouchArea` delivers THREE callbacks: `clicked`, then `clicked` + `double-clicked` on the second release. Vendored proof (`i-slint-core-1.16.1` `items/input_items.rs:154–164`): on `MouseEvent::Released`, `clicked` fires unconditionally and `double_clicked` fires additionally when `click_count % 2 == 1`. There is NO "suppress the single click while waiting for a possible double" delay — both semantics run. CONSEQUENCE for the cover-flow (`Carousel.slint` `cover-touch`): the double-click-to-open arm works precisely BECAUSE the single-click action is idempotent-friendly — normal mode's click only FOCUSES, so focus → focus → open composes naturally. In SELECTION mode the same two clicks TOGGLE the book twice (net no-op), so the `double-clicked` arm is gated `if (!root.selection-mode)` — opening mid-selection would also yank the user out of an in-progress selection. RULE: before wiring `double-clicked`, check what TWO single-click firings do first; if the single-click action is not idempotent (or its double-execution is destructive), `double-clicked` needs a gate or the single-click action needs rethinking. The open itself reuses the frozen `open(int)` callback (the Return path), so the Rust side gains no second open seam.
