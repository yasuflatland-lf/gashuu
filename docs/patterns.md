...
All five carry the same layers 2 and 4 (`border-color: Theme.glass-border`, `drop-shadow-color: Theme.shadow-popover`, `drop-shadow-blur: 22px`, `drop-shadow-offset-y: 8px`) and the same layer-3 highlight rim (a 1px `Rectangle` at `y: 1px`, inset horizontally by ~half the radius). **MANUAL SYNC CONTRACT**: any change to the glass anatomy — fill token, border color, shadow blur/offset, or rim geometry — must be applied to ALL FIVE by hand. There is no shared atom; the file and approximate line above are the grep anchor for each surface. The only copy-pasted *computation* is the rim geometry (`x: <half-radius>; y: 1px; width: parent.width - <inset>; height: 1px`); extracting it into a `GlassHighlightRim` atom was deliberately deferred to a follow-up issue — it is a mechanical 5-component refactor that was kept out of the UX-focused selection-polish PR.
