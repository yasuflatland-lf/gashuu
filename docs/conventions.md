...
Tests synthesize fixtures in memory (the `image` crate makes tiny PNGs and JPEGs) plus `tempfile` for filesystem cases — **no committed binary fixtures.** Exceptions, all committed TEXT not binaries: insta `.snap` files (see [docs/patterns.md](patterns.md)), the base64-encoded RAR `.cbr` fixtures, and the base64-encoded AVIF page fixture — all in `crates/gashuu-core/src/test_fixtures.rs`. RAR has no Rust encoder; AVIF now has none IN THIS BUILD either, because `image` is compiled without its `avif` encode feature (to keep the `ravif`/`rav1e`/`paste` encoder out of the shipped binary — see [docs/toolchain.md](toolchain.md)), so neither can be synthesized in-memory like PNGs/ZIPs. Even cheaper for PAGE-COUNTING / empty-book tests: a ZERO-BYTE file named `*.png` counts as a page, because `FolderSource` lists by EXTENSION (case-insensitive, `max_depth(1)`) and does not read the bytes — so a temp dir of empty `*.png` files probes to an N-page book, an empty dir probes to `EmptyBook`, and no real image data is needed (used by the `probe_page_count` / `probe_path` / `apply_outcomes` tests).
