...
- `crates/gashuu-core` — Slint-independent domain + I/O. Owns the `PageSource` trait (requires `Send + Sync` so `Arc<dyn PageSource>` can be shared with rayon worker threads during prefetch), `FolderSource` (top-level directory walk with natural filename ordering), `image_ops::decode` (returns raw RGBA8 + dimensions), `cache::ImageCache` (LRU of `Arc<DecodedImage>` up to `DEFAULT_CAPACITY`=50 + background ±`DEFAULT_PREFETCH_RADIUS`=3 prefetch in front of any `PageSource`), and `Settings` (`settings.rs`): persistent user settings serialized to JSON in the OS config dir via `directories::ProjectDirs`. **This is the first use of `serde` in core.** The headless boundary still holds (no slint/tracing). I/O shape: `load_from`/`save_to` take explicit paths (tempfile-testable); `load`/`save` are thin OS-path wrappers. Corrupt-file recovery (warn + fall back to defaults) lives in the UI (`main.rs`); core only returns typed `CoreError` (new variants `Settings(#[from] serde_json::Error)` and `NoConfigDir`). Errors are typed with `thiserror` (`CoreError`, `#[non_exhaustive]`).
...
- **`reading_direction`/`spread_mode`/`key_bindings` are persisted but INACTIVE in PR3** (saved for forward-compat; activation deferred to later PRs). `keymap.rs` is intentionally unchanged: `KeyBindings`'s default tokens match what `map_key` hard-codes, but `map_key` does not yet read the struct.
...
Baseline (PR1 + PR2 + PR3): top-level folder walk only (`max_depth(1)`, no recursion), LTR navigation (→/Space = next, ←/Backspace = prev), PNG/JPG/JPEG. LRU page cache (up to `DEFAULT_CAPACITY`=50 decoded images) + background ±`DEFAULT_PREFETCH_RADIUS`=3 prefetch. Settings persistence: loaded on startup / saved on exit and on folder-open; **`cache_size` and `preload_pages` are wired to real behavior**; `recent_files` recorded only when `track_recent_files` is enabled (off by default for privacy). `reading_direction`/`spread_mode`/`key_bindings` are persisted but inactive (forward-compat only).
Deferred: archive `PageSource`s → later; RTL reading → PR4; activation of persisted-but-inactive settings (`reading_direction`/`spread_mode`) + user-remappable keys; settings UI (today `track_recent_files` and other toggles require hand-editing `settings.json`).
